...
async def create_access_token(username: str, user_id: int,
                              is_admin: bool, is_supplier: bool,
                              is_customer: bool, expires_delta: timedelta):
    expires = datetime.now(timezone.utc) + expires_delta
    encode_info = {'sub': username, 'id': user_id, 'is_admin': is_admin, 'exp': expires,
                   'is_supplier': is_supplier, 'is_customer': is_customer}
    return jwt.encode(encode_info, SECRET_KEY, algorithm=ALGORITHM)
